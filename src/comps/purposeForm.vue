<template>
  <div>
    ...
    <div v-if="form.purpose === 'work'">
      <mt-cell id="cmpId" class="sel-house" title="工作单位"
        data-container="body" data-toggle="popover" data-trigger="manual" data-placement="bottom"
      >{{form.company || "请选择单位"}}</mt-cell>
      <mt-search class="search-company" v-model="searchCompany.schWords" :show="true"
        @input="onSchWdsChanged('searchCompany', ['name', 'shopName', 'address'])"
      >
        <div class="scroll-panel" style="top: 200px; bottom: 61px">
          <mt-radio align="right" v-model="form.cmpId"
            :options="searchCompany.mchItems.map(company => ({
