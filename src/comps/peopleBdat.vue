<template>
  <div class="ptb-10 plr-1pc scroll-panel" style="top: 49px; bottom: 55px">
    <div class="card ptb-10 plr-5 mb-5">
      <h2 class="text-center mb-0"><b>实有人口：{{totalNumber}}人</b></h2>
      <hr/>
...
export default {
  ...
  async created() {
    ...
    echarts.init((await utils.$wait("#purposeClassify"))[0]).setOption(utils.assignToCloneObj(utils.PieModel, {
      title: {
        text: "来此目的",
        left: "right"
      },
    ...
    echarts.init((await utils.$wait("#nationClassify"))[0]).setOption(utils.assignToCloneObj(utils.BarModel, {
      title: {text: "民族分布"},
      yAxis: {
        data: nations
      },
    ...
    echarts.init((await utils.$wait("#ageClassify"))[0]).setOption(utils.assignToCloneObj(utils.BarModel, {
      title: {text: "年龄分布"},
      yAxis: {
    ...
    echarts.init((await utils.$wait("#specialClassify"))[0]).setOption(utils.assignToCloneObj(utils.BarModel, {
      title: {text: "特殊人群分布"},
      yAxis: {
    ...
    echarts.init((await utils.$wait("#missPersonInfo"))[0]).setOption(utils.assignToCloneObj(utils.BarModel, {
      title: {text: "信息完整性统计"},
      yAxis: {
