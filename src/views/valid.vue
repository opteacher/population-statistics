<template>
  <div>
    ...
    <div class="mt-5">
      <mt-button type="primary w-98 mlr-1pc" @click="onConfirmClick"
        >确认身份</mt-button
      >
    </div>
        ...
        <div class="modal-content">
          <div class="modal-body">
            <button
              ...
              @click="onDlgBtnClick(house)"
            >
              居住房屋
            </button>
            <button
              type="button"
              class="w-100 btn btn-info mt-10"
              @click="onDlgBtnClick(company)"
            >
              工作单位
            </button>
          </div>
...
export default {
  ...
  methods: {
    ...
    async onConfirmClick() {
      ...
      let data = await utils.reqBackend(
        axios.post(url, {
          name: this.form.name,
