  ...
  <div class="scroll-panel" style="top: 49px; bottom: 55px">
    <div>
      <mt-cell class="mint-field" title="头像">
        <upload-image name="PsnPictures" v-model="form.pictures" :maxNum="3" />
      </mt-cell>
      <mt-cell class="mint-field" title="身份证照片">
        <upload-image name="PsnIdPicture" v-model="form.idPicture" />
      </mt-cell>
      ...
      <mt-cell class="mint-field" title="有无居住证">
        <mt-switch v-model="form.hasLvCard" style="color: grey">{{
          form.hasLvCard ? '有' : '无'
        }}</mt-switch>
      </mt-cell>
      <mt-cell
      ...
      <mt-cell
        title="特殊"
        :value="!showSpecial ? '展开' : '收起'"
        is-link
        @click.native="showSpecial = !showSpecial"
        data-target="#specialList"
        data-toggle="collapse"
        aria-expanded="false"
        aria-controls="specialList"
      />
    </div>
    ...
    <div class="w-100">
      ...
      <mt-button
        class="mlr-1pc mtb-1pc"
        :disabled="batchLoad"
        style="width: 98vw; display: none"
        type="default"
        @click.prevent="onBatchLoad"
        >批量导入</mt-button
      >
    </div>
  </div>
...
export default {
  ...
  methods: {
    async onSubmitClick() {
      ...
      this.form.specTags = specTagsSet
      await utils.reqBackend(
        this.form.id
          ? axios.put(
              `/population-statistics/mdl/v1/person/${this.form.id}`,
              this.form
            )
    ...
    async onBatchLoad() {
      this.batchLoad = true
      const url =
        '/population-statistics/api/v1/people/batch_load?' +
        [
          'name=姓名',
          'idCard=公民身份号码',
          'gender=性别',
      ...
      await utils.reqBackend(axios.post(url))
      Toast({
        message: '提交成功！',
        iconClass: 'iconfont icon-select-bold fs-50',
      })
