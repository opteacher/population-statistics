      ...
      <div style="position: fixed; bottom: 10px; left: 0px; right: 0px">
        ...
        >
          <svg slot="icon" class="icon" aria-hidden="true">
            <use xlink:href="#icon-download" /></svg
          >下载
        </mt-button>
        <mt-button
          class="bottom-half-btn"
          type="danger"
          @click="onDelImgClicked"
        >
          <svg slot="icon" class="icon" aria-hidden="true">
            <use xlink:href="#icon-ashbin" /></svg
          >删除
        </mt-button>
      </div>
    </mt-popup>
    <mt-button
      v-if="value.length < maxNum"
      :style="{
        width: '64px',
        height: '64px',
        'box-shadow': 'none',
...
export default {
  ...
  methods: {
    ...
    async onUpldImgChanged(e) {
      ...
      const imgURL = await reqBackend(
        axios.post(upldURL, fbody, {
    ...
    onUpldImgClicked() {
      const ipt = document.getElementById(this.elId)
      document.getElementById(this.elId).click()
    },
    onDelImgClicked() {
      MessageBox.confirm('确定删除该图片吗？', '警告')
        .then(async (action) => {
          if (action === 'confirm') {
