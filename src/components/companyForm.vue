  ...
  <div class="scroll-panel" style="top: 49px; bottom: 55px">
    <div>
      <mt-cell class="mint-field" title="单位照片">
        <upload-image name="CmpPictures" :maxNum="10" v-model="form.pictures" />
      </mt-cell>
      <mt-cell class="mint-field" title="营业执照">
        <upload-image name="CmpLicense" v-model="form.license" />
      </mt-cell>
      <mt-field
        label="单位注册名称"
      ...
      <mt-cell title="状态" class="mint-field">
        <mt-switch v-model="form.isClosed" style="color: grey">{{
          form.isClosed ? '已停业' : '营业中'
        }}</mt-switch>
      </mt-cell>
      <mt-field
        label="备注"
    ...
    <mt-cell
      title="消防安全"
      :value="!showFirefight ? '展开' : '收起'"
      is-link
      @click.native="showFirefight = !showFirefight"
      data-target="#fireFightList"
      data-toggle="collapse"
      aria-expanded="false"
      aria-controls="fireFightList"
    />
    <div class="collapse mt-3" id="fireFightList">
      ...
      <mt-cell title="高层/地下情况" class="mint-field">
        <mt-switch
          v-model="form.fireFgtTagsMap['高层/地下室']"
          style="color: grey"
        >
          {{
            form.fireFgtTagsMap['高层/地下室']
              ? '高层/地下室'
              : '非高层/非地下室'
          }}
        </mt-switch>
    ...
    <mt-cell
      title="治安安全"
      :value="!showSecurity ? '展开' : '收起'"
      is-link
      @click.native="showSecurity = !showSecurity"
      data-target="#securityList"
      data-toggle="collapse"
      aria-expanded="false"
      aria-controls="securityList"
    />
    <div class="collapse mt-3" id="securityList">
      <mt-cell title="销售酒类" class="mint-field">
        <mt-switch
          v-model="form.pbcSecuTagsMap['销售酒类']"
          style="color: grey"
        >
      ...
      <mt-cell title="存在可疑行径" class="mint-field">
        <mt-switch
          v-model="form.pbcSecuTagsMap['可疑行径']"
          style="color: grey"
        >
...
import popupField from './popupField.vue'
  ...
  methods: {
    async onSubmitClick() {
      ...
      await utils.reqBackend(
        this.form.id
          ? axios.put(
              `/population-statistics/mdl/v1/company/${this.form.id}`,
              this.form
            )
