  ...
  <div>
    ...
    >
      <div>
        <mt-cell
          title="主标题"
          :value="title.array[title.mainIdx].title"
          is-link
          @click.native="onToolBoxSelChanged('title.array', 'title.mainIdx')"
        />
        <mt-cell
          title="副标题"
          :value="title.array[title.subIdx].title"
          is-link
          @click.native="onToolBoxSelChanged('title.array', 'title.subIdx')"
        />
        <mt-cell
          title="排序"
          is-link
          :value="sort.array[sort.index].title"
          @click.native="onSortClicked"
        />
        ...
        <mt-cell
          ...
          @click.native="
            onToolBoxSelChanged('company.type.array', 'company.type.index')
          "
        />
        <mt-cell
          v-if="lsType === 'person'"
          ...
          @click.native="
            onToolBoxSelChanged('person.nation.array', 'person.nation.index')
          "
        />
        <mt-cell v-if="lsType === 'person'" title="有居住证">
          <mt-switch v-model="person.hasLvCard">{{
            person.hasLvCard ? '有' : '不筛选'
          }}</mt-switch>
        </mt-cell>
        <div v-if="lsType === 'house'">
          <mt-cell
            title="批量生成房屋"
            is-link
            :value="house.bthGenHss.show ? '收起' : '展开'"
            @click.native="house.bthGenHss.show = !house.bthGenHss.show"
            data-toggle="collapse"
            data-target="#bthGenHss"
            aria-expanded="false"
            aria-controls="bthGenHss"
          />
          <div class="collapse" id="bthGenHss">
            <mt-field
              label="地址前缀"
              placeholder="请输入地址前缀"
              v-model="house.bthGenHss.pre"
            />
            <mt-field
              label="开始室号"
              placeholder="请输入数字"
              type="number"
              v-model="house.bthGenHss.begRoom"
            />
            <mt-field
              label="结束室号"
              placeholder="请输入数字"
              type="number"
              v-model="house.bthGenHss.endRoom"
            />
            <mt-button
              type="primary"
              size="small"
              class="w-98 mlr-1pc"
              @click="onBthGenHssClicked"
              >生成</mt-button
            >
          </div>
        </div>
        <div v-if="lsType === 'company'">
          <mt-cell
            title="导出单位数据"
            is-link
            :value="company.exports.show ? '收起' : '展开'"
            @click.native="company.exports.show = !company.exports.show"
            data-toggle="collapse"
            data-target="#cmpExps"
            aria-expanded="false"
            aria-controls="cmpExps"
          />
          <div class="collapse plr-1pc" id="cmpExps">
            <mt-field
              label="导出文件名"
              placeholder="导出单位 + 日期"
              v-model="company.exports.fileName"
            />
            <mt-cell title="包含房屋">
              <mt-switch v-model="company.exports.ctnHouses">{{
                company.exports.ctnHouses ? '包含' : '不包含'
              }}</mt-switch>
            </mt-cell>
            <mt-button
              class="mt-5 w-100"
              type="primary"
              @click="onCompaniesExport"
              >导出</mt-button
            >
          </div>
        </div>
        <div v-if="lsType === 'person'">
          <mt-cell
            title="导出人员数据"
            is-link
            :value="person.exports.show ? '收起' : '展开'"
            @click.native="person.exports.show = !person.exports.show"
            data-toggle="collapse"
            data-target="#psnExps"
            aria-expanded="false"
            aria-controls="psnExps"
          />
          <div class="collapse plr-1pc" id="psnExps">
            <p class="mb-0 pl-10" style="color: #888; background-color: #ddd">
              导出列
            </p>
            <mt-cell title="姓名">
            ...
            <mt-cell
              v-for="column in person.exports.addCols"
              :key="column"
              :title="column"
            >
              <mt-button
            ...
            <mt-field
              label="导出文件名"
              placeholder="导出单位 + 日期"
              v-model="person.exports.fileName"
            />
    ...
    <div style="position: fixed; width: 100%; bottom: 0; padding: 5px 3px">
      <mt-button type="primary" style="width: 100%" @click="onToolsConfirmed"
        >确定</mt-button
      >
    </div>
  </div>
  ...
  methods: {
    ...
    onToolsConfirmed() {
      switch (this.lsType) {
        ...
        case 'person':
          if (this.person.nation.index) {
            this.searchItem.mchItems = _.filter(
              this.searchItem.allItems,
              (o) =>
                o.nation === this.person.nation.array[this.person.nation.index]
            )
          } else {
            this.searchItem.mchItems = this.searchItem.allItems
          }
        ...
        case 'company':
          if (this.company.type.index) {
            this.searchItem.mchItems = _.filter(
              this.searchItem.allItems,
              (o) => o.type === this.company.type.array[this.company.type.index]
            )
          } else {
            this.searchItem.mchItems = this.searchItem.allItems
          }
      ...
      if (this.person.hasLvCard) {
        this.searchItem.mchItems = this.searchItem.mchItems.filter(
          (item) => item.hasLvCard
        )
      }
      this.confirmed({
    ...
    onBthGenHssClicked() {
      if (
        !this.house.bthGenHss.pre.length ||
      ...
      MessageBox.confirm(
        `确定批量生成：${this.house.bthGenHss.pre}${this.house.bthGenHss.begRoom}~${this.house.bthGenHss.endRoom}室`
      )
        .then(async (action) => {
          if (action !== 'confirm') {
            return
          ...
          for (let room = nBegRm; room <= nEndRm; ++room) {
            await axios.post(
              '/population-statistics/mdl/v1/company',
              utils.copyCompany({
    ...
    async onPeopleExport() {
      ...
      const data = await utils.reqBackend(
        axios.post(url, {
          ...
          columns,
          addCols: this.person.exports.addCols,
        })
      )
      Toast({
        message: '导出Excel成功！',
    ...
    onSortClicked() {
      this.sort.index =
        this.sort.index < this.sort.array.length - 1 ? this.sort.index + 1 : 0
    },
    onAddNewColClicked() {
    ...
    onDelColClicked(column) {
      ...
      this.person.exports.addCols = this.person.exports.addCols
        .slice(0, colIdx)
        .concat(this.person.exports.addCols.slice(colIdx + 1))
    },
  },
}
</script>
