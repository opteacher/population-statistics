  ...
  <div>
    <div>
      ...
      <mt-cell
        ...
        is-link
        @click.native="
          form.relation = form.relation === '同工' ? '同住' : '同工'
        "
      />
      <mt-cell
        id="cmpId_lvAddr"
        :title="form.relation === '同工' ? '工作单位' : '居住房屋'"
        :value="
          form.relation === '同工'
            ? form.company || '请选择单位'
            : form.lvAddress || '请选择房屋'
        "
        data-container="body"
        data-toggle="popover"
        data-trigger="manual"
    ...
    <mt-search
      class="foothold-search"
      v-model="schFoothold.schWords"
      :show="true"
      @input="onSchWdsChanged('schFoothold', ['label'])"
    >
  ...
  created() {
    MessageBox('提示', this.tip).then(async () => {
      const data = await utils.reqBackend([
        axios.get(
          '/population-statistics/mdl/v1/company/s?shopName=!=&shopName='
        ), // 单位
        axios.get(
          '/population-statistics/mdl/v1/company/s?shopName===&shopName='
        ), // 房屋
      ...
      this.schFoothold.allItems =
        this.form.relation === '同工' ? this.companies : this.houses
      this.schFoothold.mchItems = this.schFoothold.allItems
    })
  },
  methods: {
    async onNextBtnClick() {
      let url = `/population-statistics/mdl/v1/person/s?name=${this.form.name}&idCard=${this.form.idCard}`
      url +=
        this.form.relation === '同工'
          ? `&cmpId=${this.form.cmpId}`
          : `&lvAddress=${this.form.lvAddress}`
